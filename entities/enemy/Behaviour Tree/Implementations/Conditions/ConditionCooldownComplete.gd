...
@export var reset_on_success : bool = true
...
func update(_actor : Node, blackboard: BehaviourTreeBlackboard) -> BehaviourState:
	var bb_ticks : int = blackboard.get_value(get_blackboard_key(), Time.get_ticks_msec())
